...
const uint32_t default_crc32_table[] = 
{
...
{
	return crc_32(array, array_size, default_crc32_table);
}
...
uint32_t crc_32(uint8_t* array, size_t array_size, const uint32_t (&table)[0x100])
{
    ...
    {
		const uint32_t lookupIndex = (crc32 ^ array[i]) & 0xFF;
		crc32 = (crc32 >> 8) ^ table[lookupIndex];  // table is an array of 256 32-bit constants
	}
