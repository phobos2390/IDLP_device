...
/// constructed using polynomial 0x5935
uint16_t default_crc16_table[0x100] =
{
...
{
    return crc_16(array, array_size,default_crc16_table);
}
...
uint16_t crc_16(uint8_t* array, size_t array_size, const uint16_t (&table)[0x100])
{
    ...
    {
		const uint16_t lookupIndex = (crc16 ^ array[i]) & 0xFF;
		crc16 = (crc16 >> 8) ^ table[lookupIndex];  // table is an array of 256 32-bit constants
	}
